...
const MainBooking = () => {
  ...
  const [reservations ,setReservations] = useState(JSON.parse(localStorage.getItem('reservations')) ? [...JSON.parse(localStorage.getItem('reservations'))] : [] );
  ...
  function updateTimes  (state , action) {
    ...
    <>
      <BookingForm fetchAPI={fetchAPI} times={availableTimes} dispatch={dispatch} formikValues={formikValues} setValues={setValues} occasions={availableOccasions}/>
        ...
        {reservations.length > 0 ? <h2 style={{textAlign:'center',margin:"10px"}}>Your reservations</h2> : null}  
      {reservations.length > 0 ?   <Button variant='outlined' style={{display:'block',margin:"10px  auto"}}  color='success' size='large' onClick={()=>navigate('/confirming')}>Go to my reservations <ArrowRight/></Button>
 : null}
        {formikValues ?  <BookingSlots setValues={setValues} formikValues={formikValues} /> : null}
        {formikValues.length > 0 ? <div> <Button variant='contained' size='large'  style={{margin:"10px"}} 
        onClick={handleClick}>
          {loading === 1 ? <CircularProgress style={{color:'white'}} size={30}/> :<p>Confirm Bookings</p>}
          </Button>
