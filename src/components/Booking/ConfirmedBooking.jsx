...
const ConfirmedBooking = () => {
  ...
  return (
    ...
    <h1 className='center'>Confirmed Bookings</h1>
        <h1 className='center'>{user.firstName} {user.lastName}'s reservations</h1>
    <div className='cardsVertical'>
      {reservationsArr.map( item => {
        return(
          <div key={uuid()} className="shopCard">
          <Card item={item}/>
          <Button variant='text' color='error' size='large' onClick={()=>deleteFunc(item)}><XLg size={50} /></Button>
          </div>
        )
      })}
      </div>

      <Button onClick={clearAll} color='error' variant='contained'>Clear all</Button>
        </main>
